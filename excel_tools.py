...
# ---------- الألوان (شكل فاخر: كحلي + ذهبي، مناسب للطباعة) ----------
INK = "000000"        # الخط الأساسي أسود
WHITE = "FFFFFF"      # أبيض
NAVY = "14213D"       # كحلي داكن لشريط العنوان
GOLD = "D9A441"       # ذهبي لرأس الجدول
GOLD_DEEP = "8F6516"  # بنّي ذهبي داكن (نص العناوين الفرعية)
SOFT_GOLD = "F4E7C8"  # ذهبي فاتح للعناوين الفرعية
ALT = "F6F6F6"        # تظليل الصفوف الفردية
TOT_GRAY = "E8D9B0"   # صف الإجمالي (ذهبي فاتح)
...
BORDER_HEAD = Border(left=THIN, right=THIN, top=MEDIUM_GOLD, bottom=THIN)
BORDER_TOT = Border(left=THIN, right=THIN, top=THIN, bottom=MEDIUM_GOLD)

F_TITLE = Font(name="Arial Black", size=16, bold=True, color=WHITE)
F_SUB = Font(name="Arial", size=11, bold=True, color=GOLD_DEEP)
...
FILL_SUB = PatternFill("solid", fgColor=SOFT_GOLD)
FILL_HEADER = PatternFill("solid", fgColor=GOLD)
...
def build_report(sheet_name, title, subtitles, headers, rows,
    ...
    """يبني ورقة تقرير فاخرة التنسيق (كحلي + ذهبي) ويرجع (wb, ws, next_row)"""
    wb = Workbook()
    ws = wb.active
    ws.title = sheet_name
    ...
    # شريط العنوان (كحلي داكن + خط ذهبي سفلي)
    ws.merge_cells(start_row=1, start_column=1, end_row=1, end_column=ncol)
    c = ws.cell(row=1, column=1, value=title)
    c.font = F_TITLE
    c.fill = FILL_TITLE
    c.alignment = CENTER
    c.border = Border(left=THIN, right=THIN, top=THIN, bottom=MEDIUM_GOLD)
    ...
    ws.row_dimensions[hdr_row].height = 27

    # البيانات (خط أسود Bold + حدود + تظليل متناوب)
    rr = hdr_row + 1
    ...
    # صف الإجمالي (ذهبي فاتح + خط سفلي ذهبي سميك)
...
def export_income(data, company, subtitle_extra="", acc_user="", signatures=None):
    ...
    data_start = len(subs) + 4
    _fin_style(ws, data_start, len(rows), ncol)
...
def export_balance(data, company, subtitle_extra="", acc_user="", signatures=None):
    ...
    wb, ws, nr = build_report("المركز المالي", "المركز المالي", subs, headers, rows,
                              num_cols=(2,), landscape=False)
    _fin_style(ws, len(subs) + 4, len(rows), ncol)
    add_signatures(ws, nr, ncol, acc_user=acc_user, signatures=signatures)
    return to_bytes(wb)
...
def export_table(sheet_name, title, headers, rows, subtitles=None, totals=None,
                 num_cols=(), date_cols=(), landscape=True, acc_user="", signatures=None):
    """تصدير عام منسّق لأي جدول — يوحد تنسيق كل تقارير الإكسل (كحلي + ذهبي + تواقيع)."""
    wb, ws, nr = build_report(sheet_name, title, subtitles or [""], headers, rows,
                              totals=totals, num_cols=num_cols, date_cols=date_cols,
                              landscape=landscape)
    add_signatures(ws, nr, len(headers), acc_user=acc_user, signatures=signatures)
    return to_bytes(wb)
